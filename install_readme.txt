    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying C and C++ standalone applications, it is 
              possible to run the shell script file run_cropDicomTool.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    All Platforms:
        * cropDicomTool.ctf (If present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * cropDicomTool.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * cropDicomTool (application created by MATLAB Compiler)
        * run_cropDicomTool.sh (shell script that can be run to 
    ...
    All Platforms:
        * cropDicomTool.dylib (shared library, file extension varies by 
          platform)
        * cropDicomTool.h (library header file)
        * cropDicomTool.ctf (If present)(Component Technology File archive,
    ...
    Windows:
        * cropDicomTool.ctf (Component Technology File archive, contains 
          compiled M-code)    
        * cropDicomTool.xml (documentation files)
        * cropDicomTool.pdb (program database file - if DEBUG option 
          selected)
        * cropDicomTool.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * cropDicomTool.ctf (Component Technology File archive, contains 
          compiled M-code)
        * cropDicomTool_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * cropDicomTool.jar (Java class files and compiled M-code)
    ...
    Windows:
        ...
        * cropDicomTool.ctf (Component Technology File archive, contains 
          compiled M-code)
        * cropDicomTool.xla (Optionally created and placed in
